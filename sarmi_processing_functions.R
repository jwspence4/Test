...
bin_scores <-
  function(col, start, end, interval) {
    ...
    names(dat) <- c("binned")
    breaks <-
      tibble(lower_bound = c(-Inf, seq(start, end, interval)),
...
plot_event_study <- function(data, y = y_, se = se_, ylabel) {
  ...
  data %>%
    ...
    labs(x = "Time to mortgage exit (years)", y = "",
...
make_income_drop_bar_two_panel <- function(cc_data, gn_data, title = "",
  ...
  comparison_data <-
    cc_data %>%
    filter(move_shock == "no move shock",
  ...
  (plot_1 <-
      ...
      geom_col(
        ...
        data = comparison_data %>%
          ...
          mutate(normalized_delta_inc =
                   ifelse(source == "Model: Utility Cost of Default",
                          NA,
                          normalized_delta_inc))
      ) +
      scale_fill_manual(values = c("#2B83BA", "#ABDDA4", "#FFFFFF"),
